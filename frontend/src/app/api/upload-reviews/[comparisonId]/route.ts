...
export async function POST(
  ...
  const upstreamUrl = `${getBackendUrl()}/api/upload-comparisons/${encodeURIComponent(comparisonId)}/reviews`;
  ...
  try {
    ...
    if (process.env.NODE_ENV !== "production") {
      console.error("Upload review proxy could not reach the backend.", { upstreamUrl, error });
    }

  ...
  return (
    <div className="space-y-5">
      ...
      {!isComparison ? <p className="rounded-md border border-slate-200 bg-white p-4 text-sm">{getCategoryLabel(item.category)}: this email does not use the BL comparison workflow.</p> : (
        <>
          {item.status === "NEEDS_REVIEW" && <p className="border-l-2 border-yellow-400 bg-yellow-50 px-3 py-2 text-sm text-yellow-900">Requires Human Review. Uncertain or missing information needs verification.</p>}

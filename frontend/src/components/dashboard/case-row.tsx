...
export function CaseRow({ item, returnTo, compact = false, prefetch = true }: { item: VerificationCase; returnTo: string; compact?: boolean; prefetch?: boolean }) {
  ...
  return (
    <li>
      <Link href={buildCaseDetailHref(item.email.email_id, returnTo)} prefetch={prefetch} className={`block px-4 hover:bg-slate-50 focus-visible:-outline-offset-4 ${compact ? "h-[84px] py-2" : "py-4"}`}>
        ...
        <div className={`flex items-end justify-between gap-3 text-xs leading-5 ${compact ? "mt-0.5" : "mt-3"}`}>
          <span className={compact ? "truncate text-slate-600" : "text-slate-600"}>{item.category !== "BL_COMPARISON" ? "Classification only" : item.status === "FAILED" ? "Processing failed" : reviewReason ? reviewReason.label : issues.length ? issues.join(" · ") : item.comparison.length === 0 ? emptyComparisonMessage : "No field issues reported"}</span>

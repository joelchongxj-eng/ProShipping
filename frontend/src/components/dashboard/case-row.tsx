...
export function CaseRow({ item, returnTo, compact = false, prefetch = true }: { item: VerificationCase; returnTo: string; compact?: boolean; prefetch?: boolean }) {
  ...
  return (
    <li>
      <Link href={buildCaseDetailHref(item.email.email_id, returnTo)} prefetch={prefetch} className={`block px-4 hover:bg-slate-50 focus-visible:-outline-offset-4 ${compact ? "h-[96px] py-2" : "py-4"}`}>

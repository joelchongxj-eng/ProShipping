  ...
  return (
    <section aria-labelledby={headingId} className="flex h-[392px] min-w-0 flex-col overflow-hidden rounded-md border border-slate-200 bg-white">
      <div className="flex h-12 shrink-0 items-center justify-between gap-3 border-b border-slate-200 px-4">
        ...
        <span aria-label={`${cases.length} cases`} className={`rounded border px-2 py-0.5 text-xs font-semibold tabular-nums ${statusStyles[group]}`}>{cases.length}</span>
      </div>
      {cases.length > 0 ? (
      ...
      <div className="flex h-12 shrink-0 items-center justify-between gap-2 border-t border-slate-200 px-4 text-xs">
        ...
        <Link href={caseListRoutes[group]} aria-label={`View all ${title} cases`} className="inline-flex min-h-9 items-center font-medium text-slate-800 underline underline-offset-4">View all <span aria-hidden="true" className="ml-1">→</span></Link>
      </div>
    </section>
  );
}

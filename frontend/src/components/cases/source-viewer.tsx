...
import type { MockDocumentSource } from "@/data/mock-document-sources";

pdfjs.GlobalWorkerOptions.workerSrc = new URL("pdfjs-dist/build/pdf.worker.min.mjs", import.meta.url).toString();

export default function SourceViewer({ source, title, initialPage, onClose }: { source: MockDocumentSource; title: string; initialPage: number | null; onClose: () => void }) {
    ...
    <dialog ref={dialog} onCancel={(event) => { event.preventDefault(); onClose(); }} aria-labelledby="source-title" className="fixed inset-0 m-auto h-[92dvh] max-h-[92dvh] w-[96vw] max-w-5xl overflow-hidden rounded-md border border-slate-300 bg-white p-0 text-slate-900 backdrop:bg-black/40">
      <div className="flex h-full min-h-0 flex-col">
        <header className="flex shrink-0 items-start justify-between gap-3 border-b border-slate-200 p-3">
          <div className="min-w-0"><h2 id="source-title" className="text-base font-semibold">{title}</h2><p className="mt-1 break-all text-xs text-slate-500">{source.filename}</p><p className="mt-1 text-xs text-slate-500">Synthetic demo document, not an original shipment file.</p></div>
          <button type="button" onClick={onClose} className={controls}>Close</button>
        </header>
        <div className="flex shrink-0 flex-wrap items-center justify-center gap-3 border-b border-slate-200 p-2">

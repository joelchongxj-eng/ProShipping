...
export function EmailSummary({ email, onClose }: { email: InboxDisplayRow; onClose: () => void }) {
  ...
  return (
    <dialog ref={ref} onCancel={(event) => { event.preventDefault(); onClose(); }} aria-labelledby="email-title" className="fixed inset-0 m-auto max-h-[90dvh] w-[94vw] max-w-xl overflow-auto rounded-md border border-slate-300 bg-white p-0 text-slate-900 backdrop:bg-slate-950/60">
      <div className="flex items-start justify-between gap-4 border-b border-slate-200 bg-slate-50 px-5 py-4"><h2 id="email-title" className="text-lg font-semibold">Email summary</h2><button type="button" onClick={onClose} className="min-h-9 rounded-md border border-slate-300 bg-white px-3 text-sm font-medium text-slate-700 shadow-sm hover:bg-slate-50">Close</button></div>
      <div className="p-5">

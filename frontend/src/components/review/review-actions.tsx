    ...
    <div>
      ...
      {action && (
        <form onSubmit={submit} aria-busy={submitting} className="mt-4 space-y-3 rounded-md border border-blue-200 bg-white p-3 shadow-sm">
          ...
          {feedback && <p role={feedback.tone === "error" ? "alert" : "status"} className={`rounded-md border px-3 py-2 text-sm ${feedback.tone === "error" ? "border-red-200 bg-red-50 text-red-800" : "border-emerald-200 bg-emerald-50 text-emerald-800"}`}>{feedback.message}</p>}

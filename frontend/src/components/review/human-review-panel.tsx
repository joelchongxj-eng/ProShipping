...
export function HumanReviewStatusBadge({ status }: { status: HumanReviewStatus }) {
  return <span className={`inline-block rounded border px-2 py-0.5 text-xs font-medium ${reviewStatusStyles[status]}`}>{humanReviewStatusLabels[status]}</span>;
}
...
function ReviewSummaryContent({ summary }: { summary: HumanReviewSummary }) {
  ...
  return (
    <div className="space-y-4">
      <dl className="grid gap-px overflow-hidden rounded border border-slate-200 bg-slate-200 sm:grid-cols-2 lg:grid-cols-4">
        <div className="bg-white p-3"><dt className="text-xs font-medium text-slate-500">Human Review Status</dt><dd className="mt-2"><HumanReviewStatusBadge status={summary.review_status} /></dd></div>
        <div className="bg-white p-3"><dt className="text-xs font-medium text-slate-500">Review Count</dt><dd className="mt-2 text-sm font-semibold tabular-nums text-slate-900">{summary.review_count}</dd></div>
        <div className="bg-white p-3"><dt className="text-xs font-medium text-slate-500">Latest Review</dt><dd className="mt-2 text-sm text-slate-800">{summary.latest_review ? reviewActionLabels[summary.latest_review.action] : "No reviews yet"}</dd>{summary.latest_review && <p className="mt-1 text-xs text-slate-500">{formatBackendTimestamp(summary.latest_review.created_at)}</p>}</div>
        <div className="bg-white p-3"><dt className="text-xs font-medium text-slate-500">Active Escalation</dt><dd className="mt-2 text-sm font-medium text-slate-800">{summary.is_escalated ? "Yes" : "No"}</dd>{summary.escalation_reason && <p className="mt-1 text-xs leading-5 text-slate-600">{summary.escalation_reason}</p>}{summary.escalated_at && <p className="mt-1 text-xs text-slate-500">{formatBackendTimestamp(summary.escalated_at)}</p>}</div>
      </dl>
...
export function HumanReviewPanel({ item, data, selectedField }: { item: VerificationCase; data: HumanReviewData; selectedField: ShippingField | null }) {
  ...
  return (
    <section aria-labelledby="human-review-title" className="rounded-md border border-slate-300 bg-white">
      <div className="border-b border-slate-200 px-4 py-3"><h2 id="human-review-title" className="text-base font-semibold text-slate-950">Human Review</h2><p className="mt-1 text-xs leading-5 text-slate-600">Automated verification remains unchanged; reviewed and effective values are recorded separately.</p></div>
      ...
      <div className="space-y-5 p-4">
        <section aria-labelledby="review-summary-title"><h3 id="review-summary-title" className="text-sm font-semibold text-slate-900">Review Summary</h3><div className="mt-2">{data.summary ? <ReviewSummaryContent summary={data.summary} /> : <SectionError message={data.errors?.summary ?? "Review summary is not available."} />}</div></section>
        <div className="grid gap-5 xl:grid-cols-2"><section aria-labelledby="review-history-title"><h3 id="review-history-title" className="text-sm font-semibold text-slate-900">Review History</h3><div className="mt-2">{data.history ? <ReviewHistoryContent history={data.history} /> : <SectionError message={data.errors?.history ?? "Review history is not available."} />}</div></section><section aria-labelledby="review-actions-title"><h3 id="review-actions-title" className="text-sm font-semibold text-slate-900">Review Actions</h3><div className="mt-2"><ReviewActions item={item} selectedField={selectedField} /></div></section></div>
        <div className="grid gap-5 xl:grid-cols-2"><section aria-labelledby="retry-history-title"><h3 id="retry-history-title" className="text-sm font-semibold text-slate-900">Retry History</h3><div className="mt-2">{data.retries ? <RetryContent retries={data.retries} /> : <SectionError message={data.errors?.retries ?? "Retry history is not available."} />}</div></section><section aria-labelledby="escalation-history-title"><h3 id="escalation-history-title" className="text-sm font-semibold text-slate-900">Escalation History</h3><div className="mt-2">{data.escalations ? <EscalationContent history={data.escalations} /> : <SectionError message={data.errors?.escalations ?? "Escalation history is not available."} />}</div></section></div>
      </div>
    </section>
  );
}

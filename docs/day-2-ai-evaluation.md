...
| `501`–`505` | TXT | `NEEDS_REVIEW` | The files named BL are invoices, packing lists, or certificates of origin. |
| `506`–`510` | Missing attachments | `NEEDS_REVIEW` | The emails request SI/BL comparison, but one or both files are absent. |
...
A later live ten-case batch (`501`–`510`) completed with no `FAILED` cases. All ten returned `NEEDS_REVIEW`: `501`–`505` for wrong document type and `506`–`510` for missing attachments. The first five BL-labeled files were inspected and contain commercial invoices, packing lists, or certificates of origin; the last five email bodies request comparison but lack one or both documents. These source checks support the ten observed review outcomes. They do not establish full-inbox accuracy.
